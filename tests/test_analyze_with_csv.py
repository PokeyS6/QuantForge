...
def test_analyze_with_csv_refuses_to_overwrite_existing_results(tmp_path):
    create_result = runner.invoke(
        app,
        [
            "create",
    ...
    project_file = tmp_path / "aapl-rsi-reversal" / "strategy.qf.json"
    first_result = runner.invoke(app, ["analyze", str(project_file), "--data-csv", str(csv_path)])
    second_result = runner.invoke(app, ["analyze", str(project_file), "--data-csv", str(csv_path)])
